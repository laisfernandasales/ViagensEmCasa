...
export async function PUT(req: NextRequest) {
  try {
    ...
    const updateData = {
      ...
      ticketPrice: parseFloat(formData.get('ticketPrice') as string),
      totalTickets: parseInt(formData.get('totalTickets') as string, 10),
      images: [...imagesToKeep, ...newImageUrls], // Combina as imagens existentes e novas
      enabled: formData.has('enabled') ? formData.get('enabled') === 'true' : existingTicketDoc.data()?.enabled, // Atualiza o estado
    };
