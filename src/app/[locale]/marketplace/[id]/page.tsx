...
interface Comment {
  ...
  rating: number;
  userName: string;
  createdAt: string; // Ensure to store as ISO string in Firestore
}
...
const useProduct = (productId: string | undefined) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [productResponse, session] = await Promise.all([
          fetch(`/api/marketplace/${productId}`),
          getSession(),
        ]);
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const response = await fetch(`/api/marketplace/${productId}`, {
        ...
        body: JSON.stringify({ text: commentText, rating, userId }),
      });
        ...
        <div className="card-body flex flex-col md:flex-row gap-8 h-[660px]">
            ...
            <div className="relative flex justify-center items-center h-96">
              ...
              {product.images.length > 1 && (
                <div className="absolute inset-y-1/2 flex justify-between w-full px-3">
                  <button onClick={() => handleImageChange(-1)} className="btn btn-circle" aria-label="Imagem anterior">
                    ❮
                  </button>
                  <button onClick={() => handleImageChange(1)} className="btn btn-circle" aria-label="Próxima imagem">
                    ❯
                  </button>
                </div>
              )}
          ...
          <div className="w-full md:w-1/2 flex flex-col justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-green-700 dark:text-green-400 mb-4 mt-2">€{product.price}</p>
              <p className="text-sm mb-4"><strong>Categoria:</strong> {product.category}</p>
              <p className="text-sm mb-4"><strong>Quantidade em Estoque:</strong> {product.stockQuantity}</p>
              <p className="text-sm mb-4"><strong>{getWeightLabel()}:</strong> {product.weight}</p>
              <p className="text-sm mb-4"><strong>Status do Produto:</strong> {product.productStatus}</p>
              <p className="text-lg mb-8 max-h-40 overflow-auto">{product.description}</p>
            ...
            <div className="flex items-center justify-between mb-4">
              <button className="btn btn-outline btn-primary w-44" onClick={() => router.push(`/${locale}/marketplace`)}>
                Voltar
              </button>
              {userRole !== 'seller' && userRole !== 'admin' && (
                <div className="flex items-center space-x-2">
                  <button onClick={() => handleQuantityChange(-1)} className="btn btn-outline btn-sm">-</button>
                  <input type="text" value={quantity} readOnly className="input input-bordered w-12 text-center" />
                  <button onClick={() => handleQuantityChange(1)} className="btn btn-outline btn-sm">+</button>
                  <button className="btn btn-primary transform transition-transform duration-200 hover:scale-105 w-44" onClick={handleAddToCart}>
          ...
          <div className="flex items-center mb-4">
            ...
            {[1, 2, 3, 4, 5].map((star) => (
              <label key={star} className="cursor-pointer">
                <input type="radio" name="rating" value={star} checked={rating === star} onChange={() => setRating(star)} className="hidden" />
                <svg className={`w-6 h-6 ${rating >= star ? 'text-yellow-500' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 .587l3.668 7.451 8.215 1.192-5.938 5.788 1.406 8.204L12 18.9l-7.351 3.872 1.406-8.204-5.938-5.788 8.215-1.192L12 .587z" />
                </svg>
              </label>
            ))}
          </div>
          <button className="btn btn-primary" onClick={handleCommentSubmit}>Enviar Comentário</button>
        </div>
